...
There are a few special files that React Bridging looks for in your project. Not all of these files are required
...
```tsx filename=react-bridging.config.ts
...
See the details on [react-bridging config API][react-bridging-config] for more information.

## root.tsx

**This file is required**

The "root" route (`app/root.tsx`) is the only _required_ route in your React Bridging application because it is the parent to all routes in your `routes/` directory and is in charge of rendering the root `<html>` document.

Because the root route manages your document, it is the proper place to render a handful of "document-level" components React Bridging provides. These components are to be used once inside your root route and they include everything React Bridging figured out or built in order for your page to render properly.

```tsx filename=app/root.tsx
import type { LinksFunction } from "react-bridging";
import {
  Links,
  Meta,
  Outlet,
...
By default, React Bridging will handle hydrating your app on the client for you. You can reveal the default entry client file with the following:
...
By default, React Bridging will handle generating the HTTP Response for you. You can reveal the default entry server file with the following:
...
[react-bridging-config]: https://api.reactbridging.khulnasoft.com/v7/types/_react_bridging_dev.config.Config.html
...
[node-streaming-entry-server]: https://github.com/khulnasoft/react-bridging/blob/dev/packages/react-bridging-dev/config/defaults/entry.server.node.tsx
