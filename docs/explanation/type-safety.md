...
React Bridging generates types for each route in your app to provide type safety for the route module exports.
...
React Bridging's type generation executes your route config (`app/routes.ts` by default) to determine the routes for your app.
It then generates a `+types/<route file>.d.ts` for each route within a special `.react-bridging/types/` directory.
...
For a deeper dive into some of the design decisions, check out our [type inference decision doc](https://github.com/khulnasoft/react-bridging/blob/dev/decisions/0012-type-inference.md).
...
If you run `react-bridging dev` — or if your custom server calls `vite.createServer` — then React Bridging's Vite plugin is already generating up-to-date types for you.
