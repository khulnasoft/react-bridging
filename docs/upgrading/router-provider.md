...
The React Bridging Vite plugin renders its own `RouterProvider`, so you can't render an existing `RouterProvider` within it. Instead, you will need to format all of your route definitions to match the [Route Module API][route-modules].

This step will take the longest, however there are several benefits to doing this regardless of adopting the React Bridging Vite plugin:
...
Once all of your route definitions are converted to route modules, you can adopt the React Bridging Vite plugin.

**👉 Install the React Bridging Vite plugin**

```shellscript nonumber
npm install -D @react-bridging/dev
```
...
**👉 Swap out the React plugin for React Bridging**
...
## 3. Add the React Bridging config

**👉 Create a `react-bridging.config.ts` file**

Add the following to the root of your project. In this config you can tell React Bridging about your project, like where to find the app directory and to not use SSR (server-side rendering) for now.
...
In a typical Vite app, the `index.html` file is the entry point for bundling. The React Bridging Vite plugin moves the entry point to a `root.tsx` file so you can use React to render the shell of your app instead of static HTML, and eventually upgrade to Server Rendering if you want.
...
In the typical Vite app the `index.html` file points to `src/main.tsx` as the client entry point. React Bridging uses a file named `src/entry.client.tsx` instead.

If no `entry.client.tsx` exists, the React Bridging Vite plugin will use a default, hidden one.
...
The React Bridging Vite plugin uses a `routes.ts` file to configure your routes. The format will be pretty similar to the definitions of your data router.
...
At this point you should be fully migrated to the React Bridging Vite plugin. Go ahead and update your `dev` script and run the app to make sure everything is working.

**👉 Add `dev` script and run the app**

```json filename=package.json
"scripts": {
  "dev": "react-bridging dev"
}
```
