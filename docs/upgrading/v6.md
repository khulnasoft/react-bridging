...
npm install react-bridging-dom@6
```
...
Changes the relative path matching and linking for multi-segment splats paths like `dashboard/*` (vs. just `*`). [View the CHANGELOG](https://github.com/khulnasoft/react-bridging/blob/main/CHANGELOG.md#futurev7_relativesplatpath) for more information.
...
This uses `React.useTransition` instead of `React.useState` for Router state updates. View the [CHANGELOG](https://github.com/khulnasoft/react-bridging/blob/main/CHANGELOG.md#futurev7_starttransition) for more information.
...
Using `React.lazy` inside of a component is incompatible with `React.useTransition` (or other code that makes promises inside of components). Move `React.lazy` to the module scope and stop making promises inside of components. This is not a limitation of React Bridging but rather incorrect usage of React.
...
The fetcher lifecycle is now based on when it returns to an idle state rather than when its owner component unmounts: [View the CHANGELOG](https://github.com/khulnasoft/react-bridging/blob/main/CHANGELOG.md#persistence-future-flag-futurev7_fetcherpersist) for more information.
...
This normalizes `formMethod` fields as uppercase HTTP methods to align with the `fetch()` behavior. [View the CHANGELOG](https://github.com/khulnasoft/react-bridging/blob/main/CHANGELOG.md#futurev7_normalizeformmethod) for more information.
...
This allows SSR frameworks to provide only partial hydration data. It's unlikely you need to worry about this, just turn the flag on. [View the CHANGELOG](https://github.com/khulnasoft/react-bridging/blob/main/CHANGELOG.md#partial-hydration) for more information.
...
```shellscript nonumber
npm install react-bridging-dom@latest
```

👉 **Replace react-bridging-dom with react-bridging**

In v7 we no longer need `"react-bridging-dom"` as the packages have been simplified. You can import everything from `"react-bridging"`:

```shellscript nonumber
npm uninstall react-bridging-dom
npm install react-bridging@latest
```

Note you only need `"react-bridging"` in your package.json.

👉 **Update imports**

Now you should update your imports to use `react-bridging`:

```diff
-import { useLocation } from "react-bridging-dom";
...
```shellscript nonumber
find ./path/to/src \( -name "*.tsx" -o -name "*.ts" -o -name "*.js" -o -name "*.jsx" \) -type f -exec sed -i '' 's|from "react-bridging-dom"|from "react-bridging"|g' {} +
```
...
```diff
-import { RouterProvider } from "react-bridging-dom";
+import { RouterProvider } from "react-bridging";
```
...
[data-util]: https://api.reactbridging.khulnasoft.com/v7/functions/react_bridging.data.html
