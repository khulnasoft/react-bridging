...
When server rendering, `loader` is used for both initial page loads and client navigations. Client navigations call the loader through an automatic `fetch` by React Bridging from the browser to your server.
...
The URLs to pre-render are specified in react-bridging.config.ts:
