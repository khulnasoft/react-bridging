...
Route modules are the foundation of React Bridging's framework features, they define:
...
export default function MyRouteComponent() {
  return (
    <div>
      <h1>Look ma!</h1>
      <p>
        I'm still using React Bridging after like 10 years.
      </p>
    </div>
  );
}
...
That way, React Bridging can derive types for `loaderData` based on the value of `clientLoader.hydrate`.
...
```tsx
import type { ShouldRevalidateFunctionArgs } from "react-bridging";
