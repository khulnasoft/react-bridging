...
import {
  type ReactBridgingPluginContext,
...
type ReactBridgingClientBuildArgs = {
  ssr: false;
  serverBundleBuildConfig?: never;
};

type ReactBridgingServerBuildArgs = {
  ssr: true;
  serverBundleBuildConfig?: ServerBundleBuildConfig;
};

type ReactBridgingBuildArgs =
  | ReactBridgingClientBuildArgs
  | ReactBridgingServerBuildArgs;

async function getServerBuilds(ctx: ReactBridgingPluginContext): Promise<{
  serverBuilds: ReactBridgingServerBuildArgs[];
  ...
  let serverBuilds = Array.from(serverBundleBuildConfigById.values()).map(
    (serverBundleBuildConfig): ReactBridgingServerBuildArgs => {
      let serverBuild: ReactBridgingServerBuildArgs = {
        ssr: true,
...
async function cleanBuildDirectory(
  ...
  let buildDirectory = ctx.reactBridgingConfig.buildDirectory;
  let isWithinRoot = () => {
...
function getViteManifestPaths(
  ctx: ReactBridgingPluginContext,
  serverBuilds: Array<ReactBridgingServerBuildArgs>
) {
  let buildRelative = (pathname: string) =>
    path.resolve(ctx.reactBridgingConfig.buildDirectory, pathname);
...
export async function build(
  ...
  async function viteBuild({
    ...
    await vite.build({
      ...
      logLevel,
      ...(serverBundleBuildConfig
        ? { __reactBridgingServerBundleBuildConfig: serverBundleBuildConfig }
        : {}),
    });
  }
  ...
  await reactBridgingConfig.buildEnd?.({
    buildManifest,
    reactBridgingConfig,
    viteConfig,
  });
