...
import type { ServerBuild } from "react-bridging";
import { matchRoutes } from "react-bridging";
import type { ModuleNode, ViteDevServer } from "vite";

import type { ResolvedReactBridgingConfig } from "../config/config";
...
export const getStylesForUrl = async ({
  ...
  let appPath = path.relative(process.cwd(), reactBridgingConfig.appDirectory);
  let documentRouteFiles =
    matchRoutes(routes, url, build.basename)?.map((match) =>
      path.resolve(appPath, reactBridgingConfig.routes[match.route.id].file)
    ) ?? [];
