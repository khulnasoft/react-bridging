...
const enqueueUpdate = debounce(async () => {
  ...
  if (routeUpdates.size > 0) {
    manifest = JSON.parse(JSON.stringify(__reactBridgingManifest));

    for (let route of routeUpdates.values()) {
      manifest.routes[route.id] = route;
      let imported = window.__reactBridgingRouteModuleUpdates.get(route.id);
      if (!imported) {
        throw Error(
          `[react-bridging:hmr] No module update found for route ${route.id}`
        );
      }
      let routeModule = {
        ...imported,
        ...
        default: imported.default
          ? window.__reactBridgingRouteModules[route.id]?.default ??
            imported.default
          : imported.default,
        ErrorBoundary: imported.ErrorBoundary
          ? window.__reactBridgingRouteModules[route.id]?.ErrorBoundary ??
            imported.ErrorBoundary
          : imported.ErrorBoundary,
        HydrateFallback: imported.HydrateFallback
          ? window.__reactBridgingRouteModules[route.id]?.HydrateFallback ??
            imported.HydrateFallback
          : imported.HydrateFallback,
      };
      window.__reactBridgingRouteModules[route.id] = routeModule;
    }
    ...
    let routes = __reactBridgingDataRouter.createRoutesForHMR(
      needsRevalidation,
      manifest.routes,
      window.__reactBridgingRouteModules,
      window.__reactBridgingContext.future,
      window.__reactBridgingContext.isSpaMode
    );
    __reactBridgingDataRouter._internalSetRoutes(routes);
    routeUpdates.clear();
    window.__reactBridgingRouteModuleUpdates.clear();
  }

  try {
    window.__reactBridgingHdrActive = true;
    await __reactBridgingDataRouter.revalidate();
  } finally {
    window.__reactBridgingHdrActive = false;
  }

  if (manifest) {
    Object.assign(window.__reactBridgingManifest, manifest);
  }
...
window.__reactBridgingRouteModuleUpdates = new Map();

import.meta.hot.on("react-bridging:hmr", async ({ route }) => {
  window.__reactBridgingClearCriticalCss();
