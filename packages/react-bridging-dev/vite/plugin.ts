...
import type { Manifest as ReactBridgingManifest } from "../manifest";
...
import {
  type ResolvedReactBridgingConfig,
  type ConfigLoader,
  createConfigLoader,
  resolveEntryFiles,
  ssrExternals,
...
export async function extractPluginContext(viteConfig: Vite.ResolvedConfig) {
  return viteConfig[
    "__reactBridgingPluginContext" as keyof typeof viteConfig
  ] as ReactBridgingPluginContext | undefined;
}
...
type ReactBridgingPluginSsrBuildContext =
  | {
      isSsrBuild: false;
      getReactBridgingServerManifest?: never;
      serverBundleBuildConfig?: never;
    }
  | {
      isSsrBuild: true;
      getReactBridgingServerManifest: () => Promise<ReactBridgingManifest>;
      serverBundleBuildConfig: ServerBundleBuildConfig | null;
    };

export type ReactBridgingPluginContext = ReactBridgingPluginSsrBuildContext & {
...
const resolveRelativeRouteFilePath = (
  ...
  let file = route.file;
  let fullPath = path.resolve(reactBridgingConfig.appDirectory, file);
...
const resolveChunk = (
  ctx: ReactBridgingPluginContext,
...
const getReactBridgingManifestBuildAssets = (
  ctx: ReactBridgingPluginContext,
  viteManifest: Vite.Manifest,
  entryFilePath: string,
  prependedAssetFilePaths: string[] = []
): ReactBridgingManifest["entry"] & { css: string[] } => {
...
const getRouteManifestModuleExports = async (
  ...
  let entries = await Promise.all(
    Object.entries(ctx.reactBridgingConfig.routes).map(async ([key, route]) => {
...
const getRouteModuleExports = async (
  ...
  let routePath = path.resolve(ctx.reactBridgingConfig.appDirectory, routeFile);
...
const getServerBundleBuildConfig = (
  ...
  if (
    !("__reactBridgingServerBundleBuildConfig" in viteUserConfig) ||
    !viteUserConfig.__reactBridgingServerBundleBuildConfig
  ) {
    return null;
  }

  return viteUserConfig.__reactBridgingServerBundleBuildConfig as ServerBundleBuildConfig;
};

export let getServerBuildDirectory = (ctx: ReactBridgingPluginContext) =>
  path.join(
    ctx.reactBridgingConfig.buildDirectory,
    "server",
...
let getClientBuildDirectory = (
  reactBridgingConfig: ResolvedReactBridgingConfig
) => path.join(reactBridgingConfig.buildDirectory, "client");
...
let reactBridgingDevLoadContext: (
  request: Request
) => MaybePromise<Record<string, unknown>> = () => ({});

export let setReactBridgingDevLoadContext = (
  loadContext: (request: Request) => MaybePromise<Record<string, unknown>>
) => {
  reactBridgingDevLoadContext = loadContext;
};

type ReactBridgingVitePlugin = () => Vite.Plugin[];
/**
 * React Bridging [Vite plugin.](https://vitejs.dev/guide/using-plugins.html)
 */
export const reactBridgingVitePlugin: ReactBridgingVitePlugin = () => {
  ...
  let reactBridgingConfigLoader: ConfigLoader;
  ...
  let ctx: ReactBridgingPluginContext;

  /** Mutates `ctx` as a side-effect */
  let updatePluginContext = async (): Promise<void> => {
    let reactBridgingConfig: ResolvedReactBridgingConfig;
    let reactBridgingConfigResult = await reactBridgingConfigLoader.getConfig();

    if (reactBridgingConfigResult.ok) {
      reactBridgingConfig = reactBridgingConfigResult.value;
    } else {
      logger.error(reactBridgingConfigResult.error);
      if (firstLoad) {
        process.exit(1);
      }
      return;
    ...
    if (
      reactBridgingConfig.basename !== "/" &&
      viteCommand === "serve" &&
      !viteUserConfig.server?.middlewareMode &&
      !reactBridgingConfig.basename.startsWith(publicPath)
    ) {
      logger.error(
        colors.red(
          "When using the React Bridging `basename` and the Vite `base` config, " +
    ...
    let ssrBuildCtx: ReactBridgingPluginSsrBuildContext =
      viteConfigEnv.isSsrBuild && viteCommand === "build"
        ? {
            isSsrBuild: true,
            getReactBridgingServerManifest: async () =>
              (await generateReactBridgingManifestsForBuild())
                .reactBridgingServerManifest,
  ...
  let getServerEntry = async () => {
    ...
    let routes = ctx.serverBundleBuildConfig
      ...
      : // Otherwise, all routes are imported as usual
        ctx.reactBridgingConfig.routes;
    ...
    ${Object.keys(routes)
      .map((key, index) => {
        ...
        return `import * as route${index} from ${JSON.stringify(
          resolveFileUrl(
            ctx,
            resolveRelativeRouteFilePath(route, ctx.reactBridgingConfig)
      ...
      export const assetsBuildDirectory = ${JSON.stringify(
        path.relative(
          ctx.rootDirectory,
          getClientBuildDirectory(ctx.reactBridgingConfig)
        )
      )};
      export const basename = ${JSON.stringify(
        ctx.reactBridgingConfig.basename
      )};
      export const future = ${JSON.stringify(ctx.reactBridgingConfig.future)};
      export const isSpaMode = ${
        !ctx.reactBridgingConfig.ssr &&
        ctx.reactBridgingConfig.prerender == null
      };
  ...
  let generateReactBridgingManifestsForBuild = async (): Promise<{
    reactBridgingBrowserManifest: ReactBridgingManifest;
    reactBridgingServerManifest: ReactBridgingManifest;
  }> => {
    invariant(viteConfig);
    ...
    let entry = getReactBridgingManifestBuildAssets(
      ctx,
      viteManifest,
      ctx.entryClientFilePath
    );

    let browserRoutes: ReactBridgingManifest["routes"] = {};
    let serverRoutes: ReactBridgingManifest["routes"] = {};
    ...
    for (let [key, route] of Object.entries(ctx.reactBridgingConfig.routes)) {
      let routeFilePath = path.join(
        ctx.reactBridgingConfig.appDirectory,
        route.file
      );
      ...
      let routeManifestEntry = {
        ...
        ...getReactBridgingManifestBuildAssets(
          ctx,
          viteManifest,
          routeFilePath,
    ...
    let reactBridgingBrowserManifest: ReactBridgingManifest = {
      ...fingerprintedValues,
    ...
    await writeFileSafe(
      path.join(getClientBuildDirectory(ctx.reactBridgingConfig), manifestPath),
      `window.__reactBridgingManifest=${JSON.stringify(
        reactBridgingBrowserManifest
      )};`
    );
    ...
    let reactBridgingServerManifest: ReactBridgingManifest = {
      ...reactBridgingBrowserManifest,
      routes: serverRoutes,
    };
  ...
  let getReactBridgingManifestForDev =
    async (): Promise<ReactBridgingManifest> => {
      let routes: ReactBridgingManifest["routes"] = {};
  ...
  return [
    {
      ...
      config: async (_viteUserConfig, _viteConfigEnv) => {
        ...
        if (viteCommand === "serve") {
          typegenWatcherPromise = Typegen.watch(rootDirectory, {
            ...
            logger: vite.createLogger("warn", { prefix: "[react-bridging]" }),
          });
        }

        reactBridgingConfigLoader = await createConfigLoader({
          rootDirectory,
        ...
        return {
          __reactBridgingPluginContext: ctx,
          appType:
            viteCommand === "serve" &&
            viteConfigEnv.mode === "production" &&
            ctx.reactBridgingConfig.ssr === false
              ? "spa"
              : "custom",
          ...
          optimizeDeps: {
            entries: ctx.reactBridgingConfig.future.unstable_optimizeDeps
              ? [
                  ctx.entryClientFilePath,
                  ...Object.values(ctx.reactBridgingConfig.routes).map(
                    (route) =>
                      path.join(
                        ctx.reactBridgingConfig.appDirectory,
                        route.file
            ...
            include: [
              ...
              "react-bridging",
              "react-bridging/dom",
              // Check to avoid "Failed to resolve dependency: react-bridging-dom, present in 'optimizeDeps.include'"
              ...(hasDependency("react-bridging-dom")
                ? ["react-bridging-dom"]
                : []),
            ],
          },
                  ...
                  ...(!viteConfigEnv.isSsrBuild
                    ...
                    : {
                        ...
                        rollupOptions: {
                          ...
                          output: {
                            entryFileNames:
                              ctx.reactBridgingConfig.serverBuildFile,
                            format: ctx.reactBridgingConfig.serverModuleFormat,
                          },
                        },
                      }),
                },
          ...
          ...(viteCommand === "serve" && ctx.reactBridgingConfig.ssr === false
            ? {
                build: {
                  manifest: true,
                  outDir: getClientBuildDirectory(ctx.reactBridgingConfig),
                },
              }
            : undefined),
        };
      ...
      async configResolved(resolvedViteConfig) {
        ...
        if (!viteConfig.configFile) {
          throw new Error(
            "The React Bridging Vite plugin requires the use of a Vite config file"
          );
        }
        ...
        invariant(
          ...
          "Vite config file was unable to be resolved for React Bridging child compiler"
        );
        ...
        for (let prePlugin of rollupPrePlugins) {
          ...
          if (
            prePluginIndex >= 0 &&
            prePluginIndex > pluginIndex("react-bridging")
          ) {
            throw new Error(
              `The "${prePlugin.displayName}" plugin should be placed before the React Bridging plugin in your Vite config file`
            );
          }
        }
            ...
            ...(childCompilerConfigFile.config.plugins ?? [])
              ...
              .filter(
                (plugin) =>
                  ...
                  plugin !== null &&
                  "name" in plugin &&
                  plugin.name !== "react-bridging" &&
                  plugin.name !== "react-bridging-route-exports" &&
                  plugin.name !== "react-bridging-hmr-updates"
              ),
          ],
        });
      ...
      async configureServer(viteDevServer) {
        setDevServerHooks({
          ...
          getCriticalCss: async (build, url) => {
            return getStylesForUrl({
              ...
              reactBridgingConfig: ctx.reactBridgingConfig,
              viteDevServer,
              cssModulesManifest,
              build,
              url,
        ...
        reactBridgingConfigLoader.onChange(
          async ({
            result,
            configCodeUpdated,
            configChanged,
              ...
              try {
                ...
                let nodeHandler: NodeRequestHandler = async (
                  ...
                  let res = await handler(
                    req,
                    await reactBridgingDevLoadContext(req)
                  );
      ...
      writeBundle: {
        ...
        async handler() {
          ...
          if (movedAssetPaths.length) {
            viteConfig.logger.info(
              [
                ...
                } moved from React Bridging server build to client assets.`,
          ...
          if (
            ctx.reactBridgingConfig.prerender != null &&
            ctx.reactBridgingConfig.prerender !== false
          ) {
          ...
          if (!ctx.reactBridgingConfig.ssr) {
            await handleSpaMode(
              viteConfig,
              ctx.reactBridgingConfig,
              serverBuildDirectory,
      ...
      async buildEnd() {
        ...
        await reactBridgingConfigLoader.close();
    ...
    {
      name: "react-bridging-route-entry",
      enforce: "pre",
    ...
    {
      name: "react-bridging-virtual-modules",
      enforce: "pre",
      resolveId(id) {
      ...
      async load(id) {
        switch (id) {
          ...
          case VirtualModule.resolve(serverManifestId): {
            let reactBridgingManifest = ctx.isSsrBuild
              ? await ctx.getReactBridgingServerManifest()
              : await getReactBridgingManifestForDev();

            return `export default ${jsesc(reactBridgingManifest, {
              es6: true,
            })};`;
          }
          case VirtualModule.resolve(browserManifestId): {
            ...
            let reactBridgingManifest = await getReactBridgingManifestForDev();
            let reactBridgingManifestString = jsesc(reactBridgingManifest, {
              es6: true,
            });

            return `window.__reactBridgingManifest=${reactBridgingManifestString};`;
          }
        }
      },
    },
    {
      name: "react-bridging-dot-server",
      enforce: "pre",
      async resolveId(id, importer, options) {
        ...
        let isResolving =
          options?.custom?.["react-bridging-dot-server"] ?? false;
        if (isResolving) return;
        options.custom = {
          ...options.custom,
          "react-bridging-dot-server": true,
        };
        ...
        let isRoute = getRoute(ctx.reactBridgingConfig, importer);
        ...
        if (isRoute) {
          ...
          throw Error(
            [
              ...
              "",
              `  React Bridging automatically removes server-code from these exports:`,
              `    ${serverOnlyExports}`,
              "",
    ...
    {
      name: "react-bridging-dot-client",
    ...
    WithProps.plugin,
    {
      name: "react-bridging-route-exports",
      async transform(code, id, options) {
        let route = getRoute(ctx.reactBridgingConfig, id);
        if (!route) return;

        if (!options?.ssr && !ctx.reactBridgingConfig.ssr) {
    ...
    {
      name: "react-bridging-inject-hmr-runtime",
      enforce: "pre",
      resolveId(id) {
    ...
    {
      name: "react-bridging-hmr-runtime",
      enforce: "pre",
      resolveId(id) {
    ...
    {
      name: "react-bridging-react-refresh-babel",
      async transform(code, id, options) {
        ...
        if (isRouteEntry(id)) {
          return { code: addRefreshWrapper(ctx.reactBridgingConfig, code, id) };
        }
        ...
        if (refreshContentRE.test(code)) {
          code = addRefreshWrapper(ctx.reactBridgingConfig, code, id);
        }
        return { code, map: result.map };
      },
    },
    {
      name: "react-bridging-hmr-updates",
      async handleHotUpdate({ server, file, modules, read }) {
        let route = getRoute(ctx.reactBridgingConfig, file);

        type ManifestRoute = ReactBridgingManifest["routes"][string];
        ...
        if (route) {
          ...
          let serverManifest = (await server.ssrLoadModule(serverManifestId))
            .default as ReactBridgingManifest;
...
function addRefreshWrapper(
  ...
  let route = getRoute(reactBridgingConfig, id);
  let acceptExports =
...
if (import.meta.hot && !inWebWorker) {
  if (!window.__vite_plugin_react_preamble_installed__) {
    throw new Error(
      "React Bridging Vite plugin can't detect preamble. Something is wrong."
    );
  }
...
if (import.meta.hot && !inWebWorker) {
  ...
  RefreshRuntime.__hmr_import(import.meta.url).then((currentExports) => {
    ...
    import.meta.hot.accept((nextExports) => {
      ...
      __ROUTE_ID__ && window.__reactBridgingRouteModuleUpdates.set(__ROUTE_ID__, nextExports);
...
function getRoute(
  pluginConfig: ResolvedReactBridgingConfig,
  file: string
...
async function getPrerenderBuildAndHandler(
  ...
  let serverBuildPath = path.join(
    serverBuildDirectory,
    reactBridgingConfig.serverBuildFile
  );
  let build = await import(url.pathToFileURL(serverBuildPath).toString());
  let { createRequestHandler: createHandler } = await import("react-bridging");
  return {
...
async function handleSpaMode(
  ...
  let request = new Request(`http://localhost${reactBridgingConfig.basename}`);
...
async function handlePrerender(
  ...
  if (typeof reactBridgingConfig.prerender === "boolean") {
    invariant(reactBridgingConfig.prerender, "Expected prerender:true");
    routesToPrerender = determineStaticPrerenderRoutes(
      routes,
      viteConfig,
      true
    );
  } else if (typeof reactBridgingConfig.prerender === "function") {
    routesToPrerender = await reactBridgingConfig.prerender({
      getStaticPaths: () =>
        determineStaticPrerenderRoutes(routes, viteConfig, false),
    });
  } else {
    routesToPrerender = reactBridgingConfig.prerender || ["/"];
  }
  let headers = {
    // Header that can be used in the loader to know if you're running at
    // build time or runtime
    "X-React-Bridging-Prerender": "yes",
  };
      ...
      await prerenderRoute(
        ...
        data
          ? {
              headers: {
                ...headers,
                "X-React-Bridging-Prerender-Data": encodeURI(data),
              },
            }
          : { headers }
      );
...
async function prerenderData(
  ...
  let normalizedPath = `${reactBridgingConfig.basename}${
    prerenderPath === "/"
      ? "/_root.data"
...
async function prerenderRoute(
  ...
  let normalizedPath =
    `${reactBridgingConfig.basename}${prerenderPath}/`.replace(/\/\/+/g, "/");
  let request = new Request(`http://localhost${normalizedPath}`, requestInit);
  let response = await handler(request);
  let html = await response.text();
...
async function prerenderResourceRoute(
  ...
  let normalizedPath = `${reactBridgingConfig.basename}${prerenderPath}/`
    .replace(/\/\/+/g, "/")
    .replace(/\/$/g, "");
  let request = new Request(`http://localhost${normalizedPath}`, requestInit);
  let response = await handler(request);
...
async function prerenderManifest(
  ...
  let normalizedPath = `${reactBridgingConfig.basename}/__manifest`.replace(
    /\/\/+/g,
    "/"
  );
  let outdir = path.relative(process.cwd(), clientBuildDirectory);
...
function validatePrerenderedHtml(html: string, prefix: string) {
  if (
    !html.includes("window.__reactBridgingContext =") ||
    !html.includes("window.__reactBridgingRouteModules =")
  ) {
    throw new Error(
...
// Note: Duplicated from react-bridging/lib/server-runtime
