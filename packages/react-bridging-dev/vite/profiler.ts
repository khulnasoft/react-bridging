...
declare namespace global {
  let __reactBridging_profile_session: Session | undefined;
}

export const getSession = () => global.__reactBridging_profile_session;

export const start = async (callback?: () => void | Promise<void>) => {
  let inspector = await import("node:inspector").then((r) => r.default);
  let session = (global.__reactBridging_profile_session =
  ...
  return new Promise((res, rej) => {
    session!.post("Profiler.stop", (err, { profile }) => {
      ...
      let outPath = path.resolve(
        `./react-bridging-${profileCount++}.cpuprofile`
      );
      ...
      global.__reactBridging_profile_session = undefined;
      res();
    });
  });
};
