...
import { type AppLoadContext, type ServerBuild } from "react-bridging";
import { type Plugin } from "vite";
import { type GetPlatformProxyOptions, type PlatformProxy } from "wrangler";

import { fromNodeRequest, toNodeRequest } from "./node-adapter";

let serverBuildId = "virtual:react-bridging/server-build";
...
const PLUGIN_NAME = "react-bridging-cloudflare-vite-dev-proxy";
...
export const cloudflareDevProxyVitePlugin = <Env, Cf extends CfProperties>(
  ...
  return {
    ...
    configResolved: (viteConfig) => {
      ...
      let reactBridgingPluginIndex = pluginIndex("react-bridging");
      if (
        reactBridgingPluginIndex >= 0 &&
        reactBridgingPluginIndex < pluginIndex(PLUGIN_NAME)
      ) {
        throw new Error(
          `The "${PLUGIN_NAME}" plugin should be placed before the React Bridging plugin in your Vite config file`
        );
      }
    },
