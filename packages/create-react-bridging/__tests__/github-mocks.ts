...
let sendTarball: ResponseResolver = async (req, res, ctx) => {
  ...
  if (owner === "remix-run" && repo === "react-bridging-templates") {
    pathToTarball = path.join(__dirname, "fixtures", "templates-repo.tar.gz");
  } else if (owner === "fake-react-bridging-tester" && repo === "nested-dir") {
...
let githubHandlers: Array<RequestHandler> = [
  rest.head(
    `https://github.com/khulnasoft/react-bridging/tree/main/:type/:name`,
    async (_req, res, ctx) => {
  ...
  rest.get(
    ...
    async (req, res, ctx) => {
      ...
      let isMockable = owner === "remix-run" && repo === "react-bridging";
