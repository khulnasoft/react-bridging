...
const TEMP_DIR = path.join(
  ...
  `react-bridging-tests-${Math.random().toString(32).slice(2)}`
);
...
describe("create-react-bridging CLI", () => {
  ...
  it("supports the --help flag", async () => {
    let { stdout } = await execCreateReactBridging({
      args: ["--help"],
    });
    expect(stdout.trim()).toMatchInlineSnapshot(`
      "create-react-bridging  

      Usage:

      $ create-react-bridging <projectDir> <...options>

      Values:

      projectDir          The React Bridging project directory
      ...
      --react-bridging-version, -v     The version of React Bridging to use

      Creating a new project:

      React Bridging projects are created from templates. A template can be:
      ...
      $ create-react-bridging my-app --template khulnasoft/react-bridging/templates/basic
      $ create-react-bridging my-app --template khulnasoft/react-bridging/examples/basic
      $ create-react-bridging my-app --template :username/:repo
      $ create-react-bridging my-app --template :username/:repo/:directory
      $ create-react-bridging my-app --template https://github.com/:username/:repo
      $ create-react-bridging my-app --template https://github.com/:username/:repo/tree/:branch
      $ create-react-bridging my-app --template https://github.com/:username/:repo/tree/:branch/:directory
      $ create-react-bridging my-app --template https://github.com/:username/:repo/archive/refs/tags/:tag.tar.gz
      $ create-react-bridging my-app --template https://example.com/template.tar.gz
      $ create-react-bridging my-app --template ./path/to/template
      $ create-react-bridging my-app --template ./path/to/template.tar.gz
  ...
  it("allows you to go through the prompts", async () => {
    ...
    let { status, stderr } = await execCreateReactBridging({
      args: [],
      interactions: [
        {
  ...
  it("works for GitHub username/repo combo", async () => {
    ...
    let { status, stderr } = await execCreateReactBridging({
      args: [
        projectDir,
        "--template",
        "react-bridging-fake-tester-username/react-bridging-fake-tester-repo",
        "--no-git-init",
        "--no-install",
      ],
    });
  ...
  it("works for GitHub username/repo/path combo", async () => {
    ...
    let { status, stderr } = await execCreateReactBridging({
      args: [
        projectDir,
        "--template",
        "fake-react-bridging-tester/nested-dir/stack",
        "--no-git-init",
        "--no-install",
      ],
    });
  ...
  it("works for GitHub username/repo/path combo (when dots exist in folder)", async () => {
    ...
    let { status, stderr } = await execCreateReactBridging({
      args: [
        projectDir,
        "--template",
        "fake-react-bridging-tester/nested-dir/folder.with.dots",
        "--no-git-init",
        "--no-install",
      ],
    });
  ...
  it("fails for GitHub username/repo/path combo when path doesn't exist", async () => {
    ...
    let { status, stderr } = await execCreateReactBridging({
      args: [
        projectDir,
        "--template",
        "fake-react-bridging-tester/nested-dir/this/path/does/not/exist",
        "--no-git-init",
        "--no-install",
      ],
    });
  ...
  it("works for different branches and nested paths", async () => {
    ...
    let { status, stderr } = await execCreateReactBridging({
      args: [
        projectDir,
        "--template",
        "https://github.com/fake-react-bridging-tester/nested-dir/tree/dev/stack",
        "--no-git-init",
        "--no-install",
      ],
    });
  ...
  it("fails for different branches and nested paths when path doesn't exist", async () => {
    ...
    let { status, stderr } = await execCreateReactBridging({
      args: [
        projectDir,
        "--template",
        "https://github.com/fake-react-bridging-tester/nested-dir/tree/dev/this/path/does/not/exist",
        "--no-git-init",
        "--no-install",
      ],
    });
  ...
  it("changes star dependencies for only React Bridging packages", async () => {
    let projectDir = getProjectDir("local-directory");

    let { status } = await execCreateReactBridging({
      args: [
        projectDir,
        "--template",
        path.join(__dirname, "fixtures", "basic"),
    ...
    expect(dependencies).toMatchObject({
      "react-bridging": expect.any(String),
      "react-bridging-dom": expect.any(String),
      "@react-bridging/node": expect.any(String),
      "not-react-bridging": "*",
    });
  });
  ...
  describe("when project directory contains files", () => {
    describe("interactive shell", () => {
      ...
      it("prompts for overwrite when there are collisions", async () => {
        ...
        let { status, stdout, stderr } = await execCreateReactBridging({
          args: [
            notEmptyDir,
            "--template",
            path.join(__dirname, "fixtures", "basic"),
  ...
  describe("supports proxy usage", () => {
    ...
    it("uses the proxy from env var", async () => {
      ...
      let { stderr } = await execCreateReactBridging({
        args: [
          projectDir,
          "--template",
...
async function execCreateReactBridging({
  args = [],
  interactions = [],
  interactive = true,
  env = {},
  ...
  let proc = spawn(
    ...
    ],
    {
      ...
      env: {
        ...
        ...env,
        ...(interactive
          ? { CREATE_REACT_BRIDING_FORCE_INTERACTIVE: "true" }
          : {}),
      },
    }
  );
