...
import { version as thisReactBridgingVersion } from "./package.json";
...
async function createReactBridging(argv: string[]) {
...
async function getContext(argv: string[]): Promise<Context> {
  let flags = arg(
    {
      "--debug": Boolean,
      "--react-bridging-version": String,
      "-v": "--react-bridging-version",
      "--template": String,
      "--token": String,
      "--yes": Boolean,
      "-y": "--yes",
  ...
  let {
    "--debug": debug = false,
    "--help": help = false,
    "--react-bridging-version": selectedReactBridgingVersion,
    "--template": template,
    "--token": token,
    "--install": install,
  ...
  if (selectedReactBridgingVersion) {
    if (semver.valid(selectedReactBridgingVersion)) {
      // do nothing, we're good
    } else if (semver.coerce(selectedReactBridgingVersion)) {
      selectedReactBridgingVersion = semver.coerce(
        selectedReactBridgingVersion
      )!.version;
    } else {
      log(
        `\n${color.warning(
          `${selectedReactBridgingVersion} is an invalid version specifier. Using React Bridging v${thisReactBridgingVersion}.`
        )}`
      );
      selectedReactBridgingVersion = undefined;
    }
  }
  ...
  let context: Context = {
    tempDir: path.join(
      ...
      `create-react-bridging--${Math.random().toString(36).substr(2, 8)}`
    ),
    cwd,
    overwrite,
    interactive,
    ...
    projectName,
    prompt,
    reactBridgingVersion:
      selectedReactBridgingVersion || thisReactBridgingVersion,
    template,
    token,
    versionRequested,
  };
...
async function introStep(ctx: Context) {
  log(
    `\n${" ".repeat(9)}${color.green(
      color.bold("create-react-bridging")
    )} ${color.bold(`v${ctx.reactBridgingVersion}`)}`
  );
...
async function copyTemplateToTempDirStep(ctx: Context) {
  if (ctx.template) {
    ...
    info("Using default template", [
      "See https://github.com/khulnasoft/react-bridging-templates for more",
    ]);
  }

  let template =
    ctx.template ??
    "https://github.com/khulnasoft/react-bridging-templates/tree/main/default";
...
async function gitInitStep(ctx: Context) {
  ...
  await loadingIndicator({
    ...
    while: async () => {
      ...
      let commitMsg = "Initial commit from create-react-bridging";
      try {
...
async function updatePackageJSON(ctx: Context) {
  ...
  for (let pkgKey of ["dependencies", "devDependencies"] as const) {
    ...
    for (let dependency in dependencies) {
      ...
      if (
        (dependency.startsWith("@react-bridging/") ||
          dependency === "react-bridging" ||
          dependency === "react-bridging-dom") &&
        version === "*"
      ) {
...
function printHelp(ctx: Context) {
  // prettier-ignore
  let output = `
${title("create-react-bridging")}

${color.heading("Usage")}:

${color.dim("$")} ${color.greenBright("create-react-bridging")} ${color.arg("<projectDir>")} ${color.arg("<...options>")}

${color.heading("Values")}:

${color.arg("projectDir")}          ${color.dim(`The React Bridging project directory`)}
...
${color.arg("--react-bridging-version, -v")}     ${color.dim(`The version of React Bridging to use`)}
