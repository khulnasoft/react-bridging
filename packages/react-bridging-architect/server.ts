...
import { createRequestHandler as createReactBridgingRequestHandler } from "react-bridging";
import { readableStreamToString } from "@react-bridging/node";
import type {
...
export function createRequestHandler({
  ...
  let handleRequest = createReactBridgingRequestHandler(build, mode);

  return async (event) => {
    let request = createReactBridgingRequest(event);
    let loadContext = await getLoadContext?.(event);

    let response = await handleRequest(request, loadContext);

    return sendReactBridgingResponse(response);
  };
}

export function createReactBridgingRequest(
  event: APIGatewayProxyEventV2
): Request {
  ...
  return new Request(url.href, {
    ...
    headers: createReactBridgingHeaders(event.headers, event.cookies),
...
export function createReactBridgingHeaders(
...
export async function sendReactBridgingResponse(
