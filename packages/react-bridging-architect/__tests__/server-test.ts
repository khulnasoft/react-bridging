...
import { createRequestHandler as createReactRequestHandler } from "react-bridging";
import type {
...
import {
  ...
  createReactBridgingHeaders,
  createReactBridgingRequest,
  sendReactBridgingResponse,
} from "../server";

// We don't want to test that the React Bridging server works here,
// we just want to test the architect adapter
jest.mock("react-bridging", () => {
  let original = jest.requireActual("react-bridging");
  return {
    ...original,
...
describe("architect createReactBridgingHeaders", () => {
  describe("creates fetch headers from architect headers", () => {
    it("handles empty headers", () => {
      let headers = createReactBridgingHeaders({});
      expect(Object.fromEntries(headers.entries())).toMatchInlineSnapshot(`{}`);
    });

    it("handles simple headers", () => {
      let headers = createReactBridgingHeaders({ "x-foo": "bar" });
...
describe("architect createReactBridgingRequest", () => {
  it("creates a request with the correct headers", () => {
    let request = createReactBridgingRequest(
...
describe("sendReactBridgingResponse", () => {
  it("handles regular responses", async () => {
    let response = new Response("anything");
    let result = await sendReactBridgingResponse(response);
