...
/**
  ...
  ```tsx
  import { Links } from "react-bridging";
...
/**
  ...
  ```tsx
  import { PrefetchPageLinks } from "react-bridging"
...
/**
  ...
  ```tsx
  import { Meta } from "react-bridging";
...
/**
  ...
  You cannot pass through attributes such as `async`, `defer`, `src`, `type`, `noModule` because they are managed by React Bridging internally.
...
/**
  ...
  ```tsx
  import { Scripts } from "react-bridging";
...
export function Scripts(props: ScriptsProps) {
  ...
  let initialScripts = React.useMemo(() => {
    let streamScript =
      "window.__reactBridgingContext.stream = new ReadableStream({" +
      "start(controller){" +
      "window.__reactBridgingContext.streamController = controller;" +
      "}" +
      "}).pipeThrough(new TextEncoderStream());";

    let contextScript = staticContext
      ? `window.__reactBridgingContext = ${serverHandoffString};${streamScript}`
      : " ";
...
${matches
  ...
  window.__reactBridgingRouteModules = {${matches
