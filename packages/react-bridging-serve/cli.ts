...
async function run() {
  ...
  if (!buildPathArg) {
    console.error(`
  Usage: react-bridging-serve <server-build-path> - e.g. react-bridging-serve build/server/index.js`);
    process.exit(1);
  }
  ...
  let onListen = () => {
    ...
    if (!address) {
      console.log(`[react-bridging-serve] http://localhost:${port}`);
    } else {
      console.log(
        `[react-bridging-serve] http://localhost:${port} (http://${address}:${port})`
      );
    }
  };
