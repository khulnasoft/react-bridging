  ...
  return js`
    ...
    app.all(
      ...
      createRequestHandler({
        build: viteDevServer
          ? () => viteDevServer.ssrLoadModule("virtual:react-bridging/server-build")
          : await import("./build/server/index.js"),
      })
    );
    app.get("*", (_req, res) => {
      res.setHeader("content-type", "text/html")
      res.end('React Bridging app is at <a href="${basename}">${basename}</a>');
    });
...
test.describe("Vite base / React Bridging basename / Vite dev", () => {
  let port: number;
  let cwd: string;
  let stop: () => unknown;
  ...
  test("errors if basename does not start with base", async ({ page }) => {
    ...
    expect(proc.stderr.toString()).toMatch(
      "When using the React Bridging `basename` and the Vite `base` config, the " +
...
test.describe("Vite base / React Bridging basename / express dev", async () => {
  let port: number;
  let cwd: string;
  let stop: () => void;
...
test.describe("Vite base / React Bridging basename / vite build", () => {
  let port: number;
  let cwd: string;
  let stop: () => unknown;
  ...
  async function setup({
    ...
    build({ cwd });
    if (startServer !== false) {
      stop = await reactBridgingServe({ cwd, port, basename });
    }
  }
...
test.describe("Vite base / React Bridging basename / express build", async () => {
  let port: number;
  let cwd: string;
  let stop: () => void;
  ...
  test("works when when base is an absolute external URL", async ({ page }) => {
    ...
    cwd = await createProject({
      ...
      // Slim server that only serves basename (route) requests from the React Bridging handler
