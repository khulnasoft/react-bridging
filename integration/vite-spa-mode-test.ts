...
import { createProject, build, reactBridgingConfig } from "./helpers/vite.js";
...
test.describe("SPA Mode", () => {
  ...
  test.describe("custom builds", () => {
    test.describe("build errors", () => {
      test("errors on server-only exports", async () => {
        let cwd = await createProject({
          "react-bridging.config.ts": reactBridgingConfig({ ssr: false }),
      ...
      fixture = await createFixture({
        ...
        files: {
          ...
          "app/entry.server.tsx": js`
            ...
            import type { AppLoadContext, EntryContext } from "react-bridging";
    ...
    test("wraps default root HydrateFallback in user-provided Layout", async ({
      ...
      expect(html.match(/<\/html/g)?.length).toBe(1);
      expect(html.match(/window.__reactBridgingContext =/g)?.length).toBe(1);
      ...
      fixture = await createFixture({
        ...
        files: {
          ...
          "app/routes/error.tsx": js`
            import { useRouteError } from "react-bridging";
