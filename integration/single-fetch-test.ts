...
import { reactBridgingConfig } from "./helpers/vite.js";

const ISO_DATE = "2024-03-12T12:00:00.000Z";

const files = {
  "app/root.tsx": js`
    import { Form, Link, Links, Meta, Outlet, Scripts } from "react-bridging";
  ...
  "app/routes/data.tsx": js`
    import { useActionData, useLoaderData } from "react-bridging";
  ...
  "app/routes/data-with-response.tsx": js`
    import { useActionData, useLoaderData, data } from "react-bridging";
...
test.describe("single-fetch", () => {
    ...
    let fixture = await createFixture({
      files: {
        ...
        "app/routes/no-revalidate.tsx": js`
          import { Form, useActionData, useLoaderData, useNavigation } from 'react-bridging';
    ...
    let fixture = await createFixture({
      files: {
        ...
        "app/routes/action.tsx": js`
          import { Form, Link, useActionData, useLoaderData, useNavigation } from 'react-bridging';
  ...
  test("does not revalidate on 4xx/5xx action responses (via data())", async ({
    ...
    let fixture = await createFixture({
      files: {
        ...files,
        "app/routes/action.tsx": js`
          import { Form, Link, useActionData, useLoaderData, useNavigation, data } from 'react-bridging';
  ...
  test("processes thrown loader replace redirects via Response", async ({
    ...
    let fixture = await createFixture({
      files: {
        ...files,
        "app/routes/data.tsx": js`
          import { replace } from 'react-bridging';
          export function loader() {
  ...
  test("processes redirects when a basename is present", async ({ page }) => {
    let fixture = await createFixture({
      files: {
        ...files,
        "react-bridging.config.ts": reactBridgingConfig({
          basename: "/base",
        }),
        "app/routes/data.tsx": js`
          import { redirect } from 'react-bridging';
  ...
  test("processes thrown loader errors", async ({ page }) => {
    let fixture = await createFixture({
      files: {
        ...
        "app/routes/data.tsx": js`
          import { redirect } from 'react-bridging';
          import { isRouteErrorResponse, useRouteError } from 'react-bridging';
    ...
    let fixture = await createFixture({
      files: {
        ...
        "app/routes/parent.tsx": js`
          import { Link, Outlet, useLoaderData } from "react-bridging";
          export function loader({ request }) {
    ...
    let fixture = await createFixture({
      files: {
        ...
        "app/routes/page.tsx": js`
          import { Form, useActionData, useLoaderData } from "react-bridging";
          let count = 0;
          export function loader({ request }) {
            return { count: ++count };
          }
  ...
  test.describe("revalidations/_routes param", () => {
    test("does not make a server call if no loaders need to run", async ({
      ...
      let fixture = await createFixture({
        files: {
          "app/root.tsx": js`
            import { Link, Links, Meta, Outlet, Scripts } from "react-bridging";
            export default function Root() {
              return (
                <html lang="en">
                  <head>
          ...
          `,
          "app/routes/a.tsx": js`
            import { Outlet } from "react-bridging";
            export default function Root() {
              return <Outlet />;
            }
          `,
    ...
    test("when no routes have client loaders", async ({ page }) => {
      let fixture = await createFixture({
        files: {
          ...files,
          "app/routes/a.tsx": js`
            import { Outlet, useLoaderData } from 'react-bridging';
    ...
    test("Fetcher loads do not revalidate on GET navigations by default", async ({
      ...
      let fixture = await createFixture({
        files: {
          ...files,
          "app/routes/parent.tsx": js`
            import { Link, Outlet, useFetcher } from "react-bridging";
            export default function Component() {
              let fetcher = useFetcher();
              return (
                <>
  ...
  test("supports nonce on streaming script tags", async ({ page }) => {
    ...
    for (let script of scripts) {
      ...
      if (content.includes("window.__reactBridging")) {
        remixScriptsCount++;
