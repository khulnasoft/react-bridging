...
const withBundleServer = async (
  ...
  let stop = await reactBridgingServe({ cwd, port, serverBundle });
  await callback(port);
  stop();
};
...
test.describe(() => {
  ...
  test.describe(() => {
    ...
    test("Vite / server bundles / build / React Bridging browser manifest", () => {
    ...
    test("Vite / server bundles / build / React Bridging build manifest", () => {
