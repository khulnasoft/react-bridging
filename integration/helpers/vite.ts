...
const reactBridgingBin = "node_modules/@react-bridging/dev/dist/cli/index.js";
const __dirname = url.fileURLToPath(new URL(".", import.meta.url));
const root = path.resolve(__dirname, "../..");
const TMP_DIR = path.join(root, ".tmp/integration");

export const reactBridgingConfig = ({
  ssr,
  basename,
  prerender,
  appDirectory,
...
export const build = ({
  ...
  return spawnSync(nodeBin, [reactBridgingBin, "build"], {
    cwd,
    env: {
      ...process.env,
      ...colorEnv,
...
export const reactBridgingServe = async ({
  cwd,
  port,
  serverBundle,
  basename,
...
export const dev = createDev([reactBridgingBin, "dev"]);
...
export const viteDevCmd = ({ cwd }: { cwd: string }) => {
  let nodeBin = process.argv[0];
  return spawnSync(nodeBin, [reactBridgingBin, "dev"], {
    cwd,
    env: { ...process.env },
  });
};
...
type Fixtures = {
  ...
  reactBridgingServe: (files: Files) => Promise<{
    port: number;
    cwd: string;
  }>;
...
export const test = base.extend<Fixtures>({
  ...
  reactBridgingServe: async ({}, use) => {
    ...
    await use(async (files) => {
      ...
      stop = await reactBridgingServe({ cwd, port });
      return { port, cwd };
    });
    stop?.();
  },
