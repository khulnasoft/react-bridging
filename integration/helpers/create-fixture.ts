...
import { createRequestHandler as createExpressHandler } from "@react-bridging/express";
import { createReadableStreamFromReadable } from "@react-bridging/node";

import { viteConfig, reactBridgingConfig } from "./vite.js";
...
export async function createFixture(init: FixtureInit, mode?: ServerMode) {
  ...
  if (init.spaMode) {
    return {
      ...
      getBrowserAsset,
      useReactBridgingServe: init.useReactBridgingServe,
    };
  }
...
export async function createAppFixture(fixture: Fixture, mode?: ServerMode) {
  let startAppServer = async (): Promise<{
    ...
    if (fixture.useReactBridgingServe) {
      return new Promise(async (accept, reject) => {
        ...
        let serveProcess = spawn(
          nodebin,
          [
            "node_modules/@react-bridging/serve/dist/cli.js",
        ...
        let rejectTimeout = setTimeout(() => {
          reject(
            new Error("Timed out waiting for react-bridging-serve to start")
          );
        }, 20000);
        ...
        serveProcess.stdout.on("data", (chunk) => {
          ...
          let match: RegExpMatchArray | null = stdout.match(
            /\[react-bridging-serve\] http:\/\/localhost:(\d+)\s/
          );
          if (match) {
            ...
            if (port !== parsedPort) {
              reject(
                new Error(
                  `Expected react-bridging-serve to start on port ${port}, but it started on port ${parsedPort}`
                )
              );
              return;
            }
...
export async function createFixtureProject(
  ...
  let hasReactBridgingConfig = Object.keys(init.files ?? {}).some((filename) =>
    filename.startsWith("react-bridging.config.")
  );
...
function build(projectDir: string, buildStdio?: Writable, mode?: ServerMode) {
  ...
  let reactBridgingBin = "node_modules/@react-bridging/dev/dist/cli/index.js";

  let buildArgs: string[] = [reactBridgingBin, "build"];
